...
def main():
    parser = argparse.ArgumentParser(description="Runner is a utility tool for running various Archer tasks.")
...
def test_file(args):
    ...
    if not is_valid_file(file):
        exit("Can only run files with '.archer' extension.")
...
def benchmark_file(args):
    ...
    if not is_valid_file(file):
        exit("Can only benchmark files with '.archer' extension.")
...
def is_valid_file(file):
    return file.rpartition('.')[-1].lower() == "archer"
